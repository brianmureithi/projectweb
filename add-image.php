				...
				<!---Brand and toggle get grouped for better mobile display--->
						<div class="navbar-header">
							...
							<div class="navbar-brand">
								<h1><a href="index.php">Upload Images  <span></span></a></h1>
							</div>
						</div>
...
<!--	<?php
$db= mysqli_connect("localhost","root","","carproject");
$sql="SELECT * FROM carimages";
$result=mysqli_query($db,$sql);
while($row = mysqli_fetch_array($result)) {
echo "<div id='img_div'>";
echo "<img src='images_path/".$row['image_path']."'>";
echo "<p>".$row['image_description']."</p>";
echo "</div>";
}
?>   -->
<div class="container">
        <form action="add-images-exec-two.php" method="post" enctype="multipart/form-data">
    <table width="100%">
        <tr>
        ...
        <tr>
            <td>Images of where?(seats,dashboard,e.t.c):</td>
            <td><input type="text" name="description" placeholder="Name(s) of the Car"/></td>
        </tr>
        <tr>
            <td colspan="2" align="center">Note: Supported image format: .jpeg, .jpg, .png, .gif</td>
        </tr>
        <tr>
            <td colspan="2" align="center"><input type="submit" value="Upload" id="selectedButton" name="upload"/></td>
        </tr>
    </table>
</form>
